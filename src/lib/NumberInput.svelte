...
<div class="number-input">
	<button class="btn decrease" on:mousedown={() => mouseDown(decrease)} on:mouseup={mouseUp}>
		&minus;
	</button>
	<input class="input" type="text" {name} bind:value on:change={() => (value = +value || 0)} />
	<button class="btn increase" on:mousedown={() => mouseDown(increase)} on:mouseup={mouseUp}>
		&plus;
	</button>
</div>
